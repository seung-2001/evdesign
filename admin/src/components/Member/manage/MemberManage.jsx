...
const MemberManage = () => {
  ...
  const fetchMembers = async (page) => {
    ...
    setTimeout(() => {
      const sampleData = Array.from({ length: 100 }, (_, i) => ({
        ...
        memberId: `user${i + 1}`,
        memberName: `홍길동${i + 1}`,
        enroleDate: new Date(2024, Math.floor(Math.random() * 12), Math.floor(Math.random() * 28) + 1)
          .toISOString().split('T')[0],
        role: ['User', 'Admin', 'Operator'][Math.floor(Math.random() * 3)],
  ...
  const handleAssignOperator = async (member) => {
    ...
    if (window.confirm(`${member.memberName}(${member.memberId})님을 관리자(Operator)로 지정하시겠습니까?`)) {
      // 실제 구현시:
        ...
        <S.TableWrapper>
          {loading ? (
            ...
            <>
              <S.Table>
                <S.TableHead>
                  <S.TableRow>
                    ...
                    <S.TableHeader>enroleDate</S.TableHeader>
                ...
                <S.TableBody>
                  {members.map((member) => (
                    <S.TableRow key={member.memberNo}>
                      ...
                      <S.TableCell $secondary>{member.enroleDate}</S.TableCell>
                      <S.TableCell>
                        <S.Badge $variant={member.role.toLowerCase()}>
                          {member.role}
                        </S.Badge>
                      </S.TableCell>
                      <S.TableCell>
                        ...
                        <S.ButtonGroup>
                          {currentUserRole === 'Admin' && (
                            <S.AssignButton
                              ...
                              disabled={!canAssignOperator(member)}
                              $canAssign={canAssignOperator(member)}
                            >
                              관리자지정
                            </S.AssignButton>
                          )}
              ...
              <S.PaginationWrapper>
                <S.PaginationInfo>
                  전체 <S.InfoNumber>{totalPages * itemsPerPage}</S.InfoNumber>개 중{' '}
                  <S.InfoNumber>{(currentPage - 1) * itemsPerPage + 1}</S.InfoNumber>-
                  <S.InfoNumber>{Math.min(currentPage * itemsPerPage, totalPages * itemsPerPage)}</S.InfoNumber> 표시
                </S.PaginationInfo>
                <S.Pagination>
                  <S.PageButton
